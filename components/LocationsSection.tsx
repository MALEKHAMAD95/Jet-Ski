...
export function LocationsSection() {
  return (
    <section id="locations" className="section-padding bg-gradient-dark">
        ...
        >
          {locations.map((loc) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className="group relative overflow-hidden rounded-3xl border border-white/10 hover:border-brand-gold/60 transition-all duration-300"
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${loc.color} opacity-30 group-hover:opacity-50 transition-opacity`} />
              ...
              <div className="relative z-10 p-8 min-h-[320px] flex flex-col">
                <motion.div
                  ...
                  className="w-14 h-14 rounded-2xl bg-gradient-gold flex items-center justify-center mb-4 shadow-lg"
                >
                  <MapPin className="w-7 h-7 text-black" strokeWidth={2.2} />
                </motion.div>
                <h3 className="text-3xl font-black uppercase text-white mb-3">{loc.name}</h3>
                <p className="text-white/80 mb-5 flex-grow">{loc.desc}</p>

                <ul className="space-y-1 mb-6">
                  {loc.highlights.map((h) => (
                    <li key={h} className="text-sm text-brand-gold/90 flex items-center gap-2">
